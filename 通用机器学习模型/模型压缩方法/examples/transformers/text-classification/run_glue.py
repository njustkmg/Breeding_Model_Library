...
"""
This script is based on HuggingFace/transformers example: https://github.com/huggingface/transformers/blob/v4.6.1/examples/pytorch/text-classification/run_glue.py
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        datasets = load_dataset("glue", data_args.task_name, cache_dir=model_args.cache_dir, keep_in_memory=data_args.keep_in_memory)
    else:
    ...
    if distill_args.distill:
        ...
        logger.info("*** Initializing teacher from {} ***".format(distill_args.teacher_name_or_path))
        logger.info("*** Distillation parameters: {}".format(distill_args))
        teacher = AutoModelForSequenceClassification.from_pretrained(distill_args.teacher_name_or_path)
        hidden_alpha = None
